...
import { Menu, Package, ShoppingCart, FileText, Users, LogOut, ChevronDown, AlertTriangle, Bell, Home, Plus, User, Calendar, Sun, Moon, TrendingUp } from 'lucide-react';
              ...
              <div className="flex items-center justify-between flex-wrap">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center mb-3">
                    {React.createElement(getGreetingIcon(), { className: "mr-3 text-blue-200 animate-pulse", size: 28 })}
                  ...
                  <p className="text-blue-100 text-lg mb-2 leading-relaxed">
                    Selamat datang kembali di <span className="font-semibold text-white">RajaPharma</span>
                  </p>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto">
                {currentLowStockItems.map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-red-50 border-l-4 border-red-500 rounded">
                    ...
                    >
                      <TrendingUp size={14} className="mr-2 group-hover:animate-pulse" />
                      <span>Restok</span>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all duration-200">
            ...
            <div className="bg-gradient-to-r from-[#1A6291] to-[#2B7CB3] p-6 rounded-t-2xl">
              <div className="flex items-center">
                ...
                <h3 className="text-xl font-bold text-white">Restok Obat</h3>
              </div>
            </div>
            ...
            <div className="p-6">
              <div className="mb-6">
                <div className="bg-gray-50 p-4 rounded-lg mb-4">
                  <p className="text-sm text-gray-600 mb-1">Nama Obat:</p>
                  <p className="font-semibold text-gray-800 text-lg">{restockModal.item?.name}</p>
                </div>
                
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="bg-red-50 p-3 rounded-lg border border-red-200">
                    <p className="text-xs text-red-600 mb-1">Stok Saat Ini</p>
                    <p className="font-bold text-red-700 text-xl">{restockModal.item?.stock}</p>
                  </div>
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                    <p className="text-xs text-blue-600 mb-1">Minimum Stok</p>
                    <p className="font-bold text-blue-700 text-xl">{restockModal.item?.minStock}</p>
                  </div>
                </div>
              </div>
                ...
                {restockAmount && (
                  <div className="mt-2 p-2 bg-green-50 rounded-lg border border-green-200">
                    <p className="text-sm text-green-700">
                      Stok setelah restok: <span className="font-bold">{restockModal.item?.stock + parseInt(restockAmount || 0)}</span>
                    </p>
                  </div>
                )}
              </div>
